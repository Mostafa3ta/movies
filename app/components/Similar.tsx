...
export default function Similar({ Recommend, detailsLink, showType = false, hideTitle = false }: { Recommend: any, detailsLink: string, showType?: boolean, hideTitle?: boolean }) {
    ...
    return (
        <div className="my-3 customScrollBar">
            {!hideTitle && <Heading center title={`Similar ${showType ? 'Shows' : 'Movies'}`} />}
            <SwiperCards spaceBetween={20} xsSlides={{ slidesPerView: 1, spaceBetween: 20 }} smSlides={{ slidesPerView: 2, spaceBetween: 20 }} mdSlides={{ slidesPerView: 3, spaceBetween: 20 }} lgSlides={{ slidesPerView: 4, spaceBetween: 20 }} pauseOnMouseEnter className='text-center px-2 items-center h-full bg-black/30 rounded-lg'
