...
export default function CastDetails({ Cast }: { Cast: CastMember[] | CrewMember[] }) {
    return <>
        <div className="my-3 mx-4 md:mx-8">
            ...
            <GridContainer className=' pb-4 lg:mx-8 items-center justify-center bg-black/40 rounded-xl overflow-auto max-h-[80vh] grid-cols-2 sm:grid-cols-3 md:grid-cols-4 xl:grid-cols-5 customScrollBar ' cols={2}>
                {Cast?.map((actor: CastMember | CrewMember, i) =>
                    <div className='flex flex-col h-full w-full my-6 items-center justify-start px-1 text-center' key={i}>
                        <CustomImg className='w-32 object-contain h-fit rounded-xl mb-2' imgSrc={actor.profile_path} />
