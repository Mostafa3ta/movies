...
export default function Seasons({ ShowDetails, ShowId, withHr = true }: { ShowDetails: any, ShowId: number, withHr?: boolean }) {
    return <>
        ...
        <SwiperCards spaceBetween={20} xsSlides={{ slidesPerView: 2, spaceBetween: 20 }} smSlides={{ slidesPerView: 2, spaceBetween: 20 }} mdSlides={{ slidesPerView: 3, spaceBetween: 20 }} lgSlides={{ slidesPerView: 4, spaceBetween: 20 }} pauseOnMouseEnter className='text-center items-center h-full px-2 bg-black/30 rounded-lg'
                ...
                card: <div className='group text-center min-h-full my-4' key={show.id} >
                    <Link href={`Season/?id=${ShowId}&season=${show.season_number}`} className='w-full h-full'>
                        <div className='flex relative overflow-hidden flex-col items-center'>
                            <CustomImg className='w-60 h-full object-contain duration-200 group-hover:scale-110 z-10 rounded-md mb-4' imgSrc={show.poster_path} />
