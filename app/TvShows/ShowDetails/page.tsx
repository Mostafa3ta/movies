...
import { CastDetails, CustomImg, DetailsLine, Empty, GridContainer, MotionItem, Rating, Seasons, Similar } from '@/app/components';
...
async function ShowDetails({ searchParams }: ParamsProps) {
    ...
    return (
        ShowDetails?.success === false ? <Empty message='Show Not Found' icon={<MdTv className="w-16 h-16 text-yellow-400" />} link="/TvShows/AllShows" linkText="Explore All Shows" /> :
            ...
            <>
                    ...
                    <GridContainer className='glass-dark rounded-2xl md:rounded-3xl items-center md:grid-cols-3 shadow-2xl shadow-fuchsia-500/20 p-4 sm:p-6 md:p-8 gap-4 sm:gap-6 md:gap-8 border border-gray-700/30' cols={1}>
                        ...
                        >
                            <div className="relative group w-full max-w-[280px] sm:max-w-xs md:max-w-none">
                                ...
                                <CustomImg priority className='relative object-cover w-full aspect-[2/3] rounded-2xl shadow-2xl' imgSrc={ShowDetails.poster_path} />
                            </div>
                        </MotionItem>
                        ...
                        >
                            ...
                            <div className="space-y-3 md:space-y-4">
                                <DetailsLine text="Genres" value={ShowDetails.genres?.map((gener: any) => (
                                ...
                                <DetailsLine text="Rating" value={<Rating rate={ShowDetails.vote_average} />} />
                                <DetailsLine text="Status" value={<span className="font-semibold text-green-400">{ShowDetails.status}</span>} />
                                ...
                                <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                                    <DetailsLine noLine text="Seasons" value={
                                        ShowDetails.number_of_seasons === 0 ? 
                                        <span className='italic text-gray-400'>Unknown</span> : 
                                        <span className='font-semibold text-fuchsia-400'>{ShowDetails.number_of_seasons}</span>
                                    } />
                                    <DetailsLine noLine text="Episodes" value={
                                        ShowDetails.number_of_episodes === 0 ? 
                                        <span className='italic text-gray-400'>Unknown</span> : 
                                        <span className='font-semibold text-fuchsia-400'>{ShowDetails.number_of_episodes}</span>
                                    } />
                                </div>
                                <Separator className="my-3 bg-gray-700/20" />
                                <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
                                    <DetailsLine noLine text="First Air Date" value={
                                        ShowDetails.first_air_date === "" ? 
                                        <span className="text-gray-400">Unknown</span> : 
                                        <span className="font-semibold">{ShowDetails.first_air_date}</span>
                                    } />
                                    <DetailsLine 
                                        noLine 
                                        text="Last Air Date" 
                                        value={
                                            ...
                                            <span className="font-semibold">{ShowDetails.last_air_date}</span>
                                        } 
                                    />
                                </div>
                            </div>
                ...
                {Simi.results.length === 0 && Recommend.results.length === 0 ? (
                    <Empty message="No Recommendations Available" icon={<MdTvOff className="w-16 h-16 text-gray-400" />} />
                ) : (
                    <>
                        {/* Recommended Shows */}
